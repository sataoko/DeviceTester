...
namespace TCPDeviceTester
{
    ...
    public class GetCRC
    {
        private static ushort[] ModBusCrcTable = {
        ...
        public static CheckSumRV Mod256(byte[] byteArray)
        {
            ...
            int crc = total - ((total % 256) * 256);

            return new CheckSumRV(crc, 2);
        }

        public static CheckSumRV XOR(byte[] byteArray)
        {
            int crc = 0;
        ...
        public static CheckSumRV CRC_CCITT_XModem(byte[] byteArray)
        {
            int crc = 0;
            int i, j;
        ...
        public static CheckSumRV CRC_CCITT_Kermit(byte[] byteArray)
        {
            int crc = 0; /* init our buffer to 0 */
            int i, j;
            byte buf;
            for (j = 0; j < byteArray.Length; j++)
            {
        ...
        public static CheckSumRV CRC_CCITTKermit0(byte[] byteArray)
        {
            int crc = 0;
            int i, j;
            for (j = 0; j < byteArray.Length; j++)
        ...
        public static CheckSumRV crc_mcl(byte[] byteArray)
        {
            int crc = 0;
            int i, j;
            for (j = 0; j < byteArray.Length; j++)
        ...
        public static CheckSumRV CRC16(byte[] byteArray)
        {
            int crc = 0;
            int i, j;
            for (j = 0; j < byteArray.Length; j++)
        ...
        public static CheckSumRV ModBus(byte[] byteArray)
        {
            ushort crc = 0xFFFF;
            foreach (byte datum in byteArray)
            {
                crc = (ushort)((crc >> 8) ^ ModBusCrcTable[(crc ^ datum) & 0xFF]);
            }
            return new CheckSumRV(crc, 2);
        }

        public static CheckSumRV SumComplement(byte[] byteArray)
        {
            byte crc = 0;
